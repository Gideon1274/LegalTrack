...
class LegalTrackLoginView(LoginView):
    ...
    def form_invalid(self, form):
        posted_email = (self.request.POST.get('username') or '').strip().lower()
        now = timezone.now()

        user = None
        if posted_email:
            user = CustomUser.objects.filter(email=posted_email).first()
        ...
        if user:
            ...
            AuditLog.objects.create(
                actor=None,
                action='login_failed',
                target_object=f"Email: {posted_email}" if posted_email else "Unknown",
                ip_address=get_client_ip(self.request),
                user_agent=self.request.META.get('HTTP_USER_AGENT', ''),
                details={'reason': 'unknown_email'} if posted_email else {'reason': 'missing_email'},
            )
